...
__attribute__((weak)) uint8_t dlt_log_level_set(uint32_t context_id, uint8_t log_level)
{
...
bool logging(uint8_t context_id, uint8_t log_level, uint8_t *data, uint16_t len)
{
    dlt_log_t *target = log_group_search(context_id);
    if (target == NULL)
    {
    ...
    if (target->log_level >= log_level)
    {   printf("target_log_level:%d, log_level:%d\n",target->log_level , log_level);
        gdcn_core_handle_t gdcn_handle;
        display_EVENT_DLT(gdcn_handle, MESSAGE_ID, data, len);
    }
}
 ...
 ****************************************************************************/
// void dlt_register(uint8_t app_id, uint8_t context_id)
...
void debug_and_trace_init(void)
{
    ...
    for (uint8_t i = 0; i < MAX_MODULE_NUM; ++i)
    {
        log_manager[i].log_level = LOG_LEVEL_DEFA;
    }
}
...
uint8_t dlt_init(void)
{
    ...
    for (uint8_t i = 0; i < MAX_MODULE_NUM; ++i)
    {
        log_group[i].log_level = LOG_LEVEL_DEFA;
    }
...
bool dlt_console(int argc, char *argv[])
{
    ...
    {
        uint8_t buf[] = "LOG_XXXX";
        memcpy(buf, "LOG_FATA", sizeof(buf));        LOG_FATA(2, buf, sizeof(buf));
        memcpy(buf, "LOG_ERRO", sizeof(buf));        LOG_ERRO(2, buf, sizeof(buf));
        memcpy(buf, "LOG_WARN", sizeof(buf));        LOG_WARN(3, buf, sizeof(buf));
        memcpy(buf, "LOG_INFO", sizeof(buf));        LOG_INFO(3, buf, sizeof(buf));
        memcpy(buf, "LOG_DEBU", sizeof(buf));        LOG_DEBU(4, buf, sizeof(buf));
        memcpy(buf, "LOG_VERB", sizeof(buf));        LOG_VERB(4, buf, sizeof(buf));
    }
    else if (!STRNCMP(argv[1], "level"))
    {
        uint8_t context_id = strtol(argv[2], NULL, 16);
        uint8_t level = strtol(argv[3], NULL, 16);
        dlt_log_level_set(context_id, level);
    }
